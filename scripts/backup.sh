...
mkdir -p "$BACKUP_DIR"
# Back up wiki + outputs (content only, not raw/ which is ephemeral)
tar -czf "$BACKUP_DIR/vault-$STAMP.tar.gz" -C "$VAULT" wiki outputs 2>/dev/null
...
cd "$VAULT"
git add wiki outputs/cost-log.md .gitignore 2>/dev/null || true
